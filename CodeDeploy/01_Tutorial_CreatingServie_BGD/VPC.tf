...
resource "aws_subnet" "Subnet_DEV_Public03" {
  vpc_id = aws_vpc.VPC.id
  cidr_block = "10.7.3.0/24"
  availability_zone = data.aws_availability_zones.available.names[3]
  tags = {
    Name = "IB07441-SubnetDEVPublic03-TCL"
  }
}
...
resource "aws_subnet" "Subnet_DEV_Private03" {
  vpc_id = aws_vpc.VPC.id
  cidr_block = "10.7.13.0/24"
  availability_zone = data.aws_availability_zones.available.names[3]
  tags = {
    Name = "IB07441-SubnetDEVPrivate03-TCL"
  }
}
...
resource "aws_route_table_association" "RouteTableAssociation_SubnetDEVPublic03" {
  subnet_id = aws_subnet.Subnet_DEV_Public03.id
  route_table_id = aws_route_table.RouteTable_DEV_Public.id
}
...
#   route_table_id = aws_route_table.RouteTable_DEV_Private.id
# }
